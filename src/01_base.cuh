    ...
    int N)
{
    ...
    // Prologue: load Q — stays in SMEM for entire mainloop
    load_tile_async<BR, D, NUM_THREADS>(Q_block_ptr, smem_Q, D, tid);
    cp_async_commit();
    cp_async_wait<0>();
    __syncthreads();
    ...
    for (int j = 0; j < n_kv_blocks; j++) {
        load_tile_async<BC, D, NUM_THREADS>(K_head + j * BC * D, smem_K, D, tid);
        load_tile_async<BC, D, NUM_THREADS>(V_head + j * BC * D, smem_V, D, tid);
        cp_async_commit();
        cp_async_wait<0>();
        __syncthreads();
        ...
        for (int mt = 0; mt < M_TILES; mt++) {
            ...
            gemm_qk_smemq_base<QK_K_TILES, QK_N_TILES, D>(
            ...
            gemm_pv_regp<QK_N_TILES, PV_N_TILES, D>(
    ...
    #pragma unroll
    for (int mt = 0; mt < M_TILES; mt++)
        epilogue_scatter<D, PV_N_TILES>(
            O_accum[mt], &l[mt * 2], smem_Q,
            warp_base_row + mt * MMA_M);

    __syncthreads();

    epilogue_store<BR, D, NUM_THREADS>(
