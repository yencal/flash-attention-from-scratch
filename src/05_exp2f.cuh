    ...
    int N)
{
    ...
    for (int j = 0; j < n_kv_blocks; j++) {
        ...
        // Phase 1: QK + fused-scale exp2f softmax + convert P
        ...
        // Phase 2: PV GEMM with x4.trans V pair loading
        #pragma unroll
        for (int mt = 0; mt < M_TILES; mt++) {
            gemm_pv_x4trans<PV_K_TILES, PV_N_TILES, D>(
